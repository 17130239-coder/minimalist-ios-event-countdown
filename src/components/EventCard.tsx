...
export const EventCard: React.FC<EventCardProps> = ({
  ...
  return (
    <article
      onClick={() => onSelect(event)}
      className="gallery-card relative min-h-[380px] rounded-3xl overflow-hidden cursor-pointer group border border-slate-200/80 dark:border-white/10 bg-slate-900 flex flex-col justify-between p-6 shadow-[0_4px_20px_rgba(0,0,0,0.06)] dark:shadow-2xl"
    >
      <img
        src={event.imageUrl}
        alt={event.name}
      ...
      <div className="relative z-10 flex items-center justify-between">
        <span className="text-[11px] font-semibold uppercase tracking-wider px-3 py-1 rounded-full bg-black/50 backdrop-blur-md text-white border border-white/20 flex items-center gap-1.5 shadow-sm">
          <span className="material-symbols-outlined text-[14px]">{catMeta.icon}</span>
          {catMeta.label}
        </span>
        <span className="w-8 h-8 rounded-full bg-white/20 backdrop-blur-md flex items-center justify-center text-white group-hover:bg-white group-hover:text-black transition-all shadow-sm">
          <span className="material-symbols-outlined text-[16px]">north_east</span>
        </span>
      </div>
      ...
      <div className="relative z-10 flex flex-col gap-3.5 mt-auto">
        <h2 className="text-2xl font-bold text-white tracking-tight line-clamp-2 drop-shadow-sm">
          {event.name}
        </h2>

        <div className="flex items-center justify-between bg-white/95 dark:bg-black/85 backdrop-blur-xl border border-slate-200/80 dark:border-white/15 px-3.5 py-2.5 rounded-2xl shadow-xl">
